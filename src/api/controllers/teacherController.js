...
const getStudentCourseScores = async (req, res) => {
  ...
  const getQuizPredictions = async (req, res) => {
    try {
      ...
      const gradioPostStart = Date.now();
      ...
      console.log(`Gradio POST request took ${Date.now() - gradioPostStart} ms`);
      ...
      while (attempts < maxAttempts) {
        console.log(`Waiting 1 second before GET attempt ${attempts + 1}`);
        await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second
  
        const gradioGetStart = Date.now();
        ...
        console.log(`Gradio GET attempt ${attempts + 1} took ${Date.now() - gradioGetStart} ms`);
